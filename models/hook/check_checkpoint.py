...
class Check_CheckpointHook(Hook):
    ...
    def before_run(self, runner):
        ...
        runner.logger.info(f'Checkpoints Hook is Check_Checkpoints [Custom Hook for Double Check].')
        runner.logger.info(f'**********************************************************************************')
    ...
    def _save_checkpoint(self, runner):
        ...
        if runner.meta is not None:
            ...
            # Check pth files
            global_checkpoint_path = runner.meta['hook_msgs']['last_ckpt'] # type str
            attempts  = 0
            max_attempts  = 10
            while attempts < max_attempts :
                if self.checkpoint_check(global_checkpoint_path):
                    break
                else:
                    runner.logger.info(
                        f'Checkpoint error at {runner.iter + 1} iterations')
                    # Re-Save
                    runner.save_checkpoint(
                        self.out_dir, save_optimizer=self.save_optimizer, **self.args)
                attempts  = attempts  + 1
            if attempts  == max_attempts:
                runner.logger.info(
                    f'Saving checkpoint error at {runner.iter + 1} iterations [Max Attempts 10]')
    ...
    def checkpoint_check(self, path):
        try:
