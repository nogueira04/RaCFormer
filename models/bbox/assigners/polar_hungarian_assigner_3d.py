...
class PolarHungarianAssigner3D(BaseAssigner):
    def __init__(self,
                 ...
                 theta_cost=dict(type='ThetaL1Cost', weight=1.0),
                 iou_cost=dict(type='IoUCost', weight=0.0),
                 pc_range=None):
        self.cls_cost = build_match_cost(cls_cost)
        self.reg_cost = build_match_cost(reg_cost)
        self.theta_cost = build_match_cost(theta_cost)
    ...
    def assign(self,
        ...
        theta_cost = self.theta_cost(bbox_pred, normalized_gt_bboxes)
        
        # weighted sum of above two costs
        cost = cls_cost + reg_cost + theta_cost
