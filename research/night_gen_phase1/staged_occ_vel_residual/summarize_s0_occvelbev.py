...
def main():
    ...
    payload = {
        ...
        "delta_vs_s0": deltas,
        "gate_vs_s0": s0_gate,
        "note": "Zero-init radar occupancy plus compensated velocity BEV residual with RCS muted.",
    }
    (out_dir / "summary_metrics.json").write_text(json.dumps(payload, indent=2, sort_keys=True))

    lines = [
        "# S0 occupancy + velocity BEV residual summary",
        "",
        "Zero-init radar occupancy, vx_comp, and vy_comp BEV residual trained on the S0 day-only subset.",
        "",
