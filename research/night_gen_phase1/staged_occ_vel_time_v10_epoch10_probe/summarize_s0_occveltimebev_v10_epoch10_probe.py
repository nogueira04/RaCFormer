...
STAGE = "S0_occveltimebev_v10_epoch10_probe"
BASELINE = "S0"
...
def main():
    ...
    payload = {
        ...
        "note": (
            "Parallel epoch-10 probe for zero-init radar occupancy plus compensated velocity and "
            "sweep-time BEV residual with RCS muted and vx/vy scales set to 10.0."
        ),
    }
    (out_dir / "summary_metrics.json").write_text(json.dumps(payload, indent=2, sort_keys=True))

    lines = [
        "# S0 occupancy + velocity + time BEV residual v10 epoch-10 probe",
        "",
        "Parallel epoch-10 condition-eval probe using the v10 checkpoint before final epoch-12 evaluation completes.",
        "",
