...
def main():
    ...
    payload = {
        ...
        "delta_vs_s0": deltas,
        "gate_vs_s0": s0_gate,
        "note": "Zero-init radar RCS plus compensated velocity and sweep-time BEV residual.",
    }
    (out_dir / "summary_metrics.json").write_text(json.dumps(payload, indent=2, sort_keys=True))

    lines = [
        "# S0 RCS + velocity + time BEV residual summary",
        "",
        "Zero-init radar RCS, vx_comp, vy_comp, and sweep-time BEV residual trained on the S0 day-only subset.",
        "",
