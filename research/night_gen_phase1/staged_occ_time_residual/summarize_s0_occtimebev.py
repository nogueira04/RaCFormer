...
def main():
    ...
    payload = {
        ...
        "delta_vs_s0": deltas,
        "gate_vs_s0": s0_gate,
        "note": "Zero-init radar occupancy plus sweep-time BEV residual with RCS muted and compensated velocity removed.",
    }
    (out_dir / "summary_metrics.json").write_text(json.dumps(payload, indent=2, sort_keys=True))

    lines = [
        "# S0 occupancy + time BEV residual summary",
        "",
        "Zero-init radar occupancy and sweep-time BEV residual trained on the S0 day-only subset, with RCS muted and compensated velocity removed.",
        "",
