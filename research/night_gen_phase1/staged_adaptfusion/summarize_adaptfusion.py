...
BASELINE = "S0"
REFERENCE = "S3_seed20260425_ratio18p75_w05"
...
def main():
    ...
    lines = [
        "# S3 seed20260425 ratio18p75 w05 adaptive-fusion summary",
        "",
        "Adaptive fusion gate enabled on top of the corrected w05 generated-sample weighting branch.",
        "",
        "| Split | mAP | NDS | mAP vs S0 | NDS vs S0 | mAP vs w05 | NDS vs w05 |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for split in splits:
        lines.append(
            "| {split} | {map:.4f} | {nds:.4f} | {dmap_s0} | {dnds_s0} | {dmap_ref} | {dnds_ref} |".format(
                ...
                dmap_ref=fmt_pp(payload["delta_vs_w05"][split]["mAP"]),
                dnds_ref=fmt_pp(payload["delta_vs_w05"][split]["NDS"]),
            )
        )
    ...
    ref_night_delta = payload["delta_vs_w05"]["night"]["mAP"]
    ref_day_delta = payload["delta_vs_w05"]["day"]["mAP"]
    ref_overall_delta = payload["delta_vs_w05"]["overall"]["mAP"]
    ref_night_nds = payload["delta_vs_w05"]["night"]["NDS"]
    ref_gate = (
        ref_night_delta >= 0.0
    ...
    lines.extend(
        [
            "",
            f"Gate verdict vs w05: {'PASS' if ref_gate else 'FAIL'}",
            f"Gate verdict vs S0: {'PASS' if s0_gate else 'FAIL'}",
            "",
            "Gate vs w05: night mAP retained/improved, day mAP >= -1.0 pp, overall mAP >= -1.5 pp, night NDS >= -0.5 pp.",
