...
CANDIDATE = "full_nightaug_epoch36_condition"
...
def main():
    ...
    payload = {
        "baseline": BASELINE,
        "candidate": CANDIDATE,
        "metrics": {"baseline": baseline, "nightaug": candidate},
        "delta_nightaug_vs_baseline": deltas,
        "gate": "PASS" if gate else "FAIL",
        "note": "Legacy full-training checkpoint comparison; not a train2k NB2 result.",
    }
    (ROOT / "full_nightaug_epoch36_comparison.json").write_text(json.dumps(payload, indent=2, sort_keys=True))

    lines = [
        "# Full Epoch-36 NightAug Comparison",
        "",
        "Legacy full-training checkpoint comparison. Use as evidence for night-augmentation direction, not as the train2k NB2 gate result.",
        "",
        "| Split | Baseline mAP | NightAug mAP | Delta mAP | Baseline NDS | NightAug NDS | Delta NDS |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for split in splits:
        lines.append(
    ...
    (ROOT / "full_nightaug_epoch36_comparison.md").write_text("\n".join(lines) + "\n")
    print(ROOT / "full_nightaug_epoch36_comparison.md")
