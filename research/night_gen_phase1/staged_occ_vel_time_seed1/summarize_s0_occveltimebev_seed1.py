...
def main():
    ...
    payload = {
        ...
        "delta_vs_s0": deltas,
        "gate_vs_s0": s0_gate,
        "note": "Seed-1 confirmation for zero-init radar occupancy plus compensated velocity and sweep-time BEV residual with RCS muted.",
    }
    (out_dir / "summary_metrics.json").write_text(json.dumps(payload, indent=2, sort_keys=True))

    lines = [
        "# S0 occupancy + velocity + time BEV residual seed-1 summary",
        "",
        "Seed-1 confirmation of zero-init radar occupancy, vx_comp, vy_comp, and sweep-time BEV residual trained on the S0 day-only subset.",
        "",
