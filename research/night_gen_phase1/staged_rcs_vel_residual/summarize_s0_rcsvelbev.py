...
def main():
    ...
    payload = {
        ...
        "delta_vs_s0": deltas,
        "gate_vs_s0": s0_gate,
        "note": "SGDet3D-inspired zero-init radar occupancy+RCS+velocity BEV residual.",
    }
    (out_dir / "summary_metrics.json").write_text(json.dumps(payload, indent=2, sort_keys=True))

    lines = [
        "# S0 RCS + velocity BEV residual summary",
        "",
        "Zero-init radar occupancy, RCS, vx_comp, and vy_comp BEV residual trained on the S0 day-only subset.",
        "",
