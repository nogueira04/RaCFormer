...
def main():
    ...
    payload = {
        ...
        "delta_vs_s0": deltas,
        "gate_vs_s0": s0_gate,
        "note": "Early epoch-1 screen for the SGDet3D-inspired occupancy+RCS+velocity BEV residual.",
    }
    (out_dir / "summary_metrics.json").write_text(json.dumps(payload, indent=2, sort_keys=True))

    lines = [
        "# S0 RCS + velocity BEV residual epoch-1 screen",
        "",
        "Early checkpoint screen only. Use this as a sanity signal, not as final promotion evidence.",
        "",
