...
def main():
    ...
    payload = {
        ...
        "delta_vs_s0": deltas,
        "gate_vs_s0": s0_gate,
        "note": "Zero-init radar occupancy plus compensated velocity and sweep-time BEV residual with RCS muted and vx/vy scales set to 10.0.",
    }
    (out_dir / "summary_metrics.json").write_text(json.dumps(payload, indent=2, sort_keys=True))

    lines = [
        "# S0 occupancy + velocity + time BEV residual v10 summary",
        "",
        "Zero-init radar occupancy, vx_comp, vy_comp, and sweep-time BEV residual with vx/vy scales set to 10.0.",
        "",
