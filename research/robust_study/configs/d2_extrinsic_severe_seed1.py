# (d2) extrinsic miscalibration, severe severity, seed 1
...
#     --config research/robust_study/configs/d2_extrinsic_severe_seed1.py \
...
# (d2) extrinsic miscalibration, level severe (sigma_rot=0.1, sigma_trans=0.01),
# corruption-realisation seed 1. Draws are keyed by (seed, scene_token, radar channel)
# and severity scales the same draws, so this cell shares common random numbers with
# d2_extrinsic_medium_seed1.py.
_pipeline = build_pipeline(
    frame_t_type="D2MiscalibLoadnuradarpoints",
    sweeps_type="D2MiscalibLoadradarpointsFromMultiSweeps",
    extra=dict(level="severe", seed=1),
)
