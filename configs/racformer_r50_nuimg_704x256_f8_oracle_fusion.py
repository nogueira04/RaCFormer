"""
RaCFormer Oracle Test Configuration

This config enables the Oracle Fusion Test from PATH_B_DEEP_DIVE.md Section 3.1.

The Oracle Test manually sets fusion weights based on scene conditions:
...
Purpose: Verify that adaptive fusion weights CAN help before implementing
the full learning-based uncertainty-aware fusion.

Success criteria: If oracle weights improve night mAP by >1%, learned weights should work.
"""

# Import the base config and override specific settings
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        transformer=dict(
            ...
            oracle_fusion=True),  # ORACLE TEST: Enable condition-based adaptive fusion
        bbox_coder=dict(
            type='NMSFreeCoder',
...
# ORACLE TEST: Add scene_condition to meta_keys for fusion weight selection
train_pipeline = [
    ...
    dict(type='Collect3D', keys=['gt_bboxes_3d', 'gt_labels_3d', 'img', 'gt_depth', 'radar_depth', 'radar_rcs', 'radar_points'], meta_keys=(
        'filename', 'ori_shape', 'img_shape', 'pad_shape', 'lidar2img', 'img_timestamp', 'intrinsics', 'scene_condition'))  # Added scene_condition
]
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='Collect3D', keys=['img', 'gt_depth', 'radar_points', 'radar_depth', 'radar_rcs'], meta_keys=(
                ...
                'lidar2img', 'img_timestamp', 'intrinsics', 'scene_condition'))  # Added scene_condition
        ])
]
