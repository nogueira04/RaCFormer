"""
RaCFormer config with Night Augmentation for improved night-time performance.

Based on: racformer_r50_nuimg_704x256_f8.py
Added: SimulateNight augmentation with enhanced features (vignette, headlight gradient,
       bright spots, preserve bright regions) to improve model robustness to night conditions.

Night augmentation statistics (validated against real nuScenes night images):
- Real night: mean_brightness=36.9, std=34.0
- Enhanced medium preset: mean_brightness=37.8, std=40.1
- Contrast gap reduced from 9.2 (basic) to 6.1 (enhanced)
"""
...
train_pipeline = [
    ...
    # Night augmentation - simulates night conditions on daytime images
    # Parameters tuned to match real nuScenes night image statistics
    # Applied AFTER resize (256x704) instead of before (900x1600) for ~8x faster processing
    dict(type='SimulateNight',
         prob=0.3,  # Apply to 30% of training samples
         # Base parameters - tuned between enhanced_light and enhanced_medium
         brightness_range=(0.42, 0.52),
         gamma_range=(1.5, 1.9),
         contrast_range=(0.68, 0.78),
         noise_std_range=(6, 12),
         # Color shift for streetlight effect
         color_shift=True,
         color_shift_strength=(0.1, 0.18),
         # Enhanced features for realistic night simulation
         vignette=True,
...
data = dict(
    ...
    train=dict(
        ...
        ann_file='/srv/nfs/shared/gnmp/RaCFormer/nuscenes_infos_train_sweep.pkl',
...
resume_from = 'outputs/racformer_r50_nuimg_704x256_f8_nightaug/2026-02-01/01-41-59/epoch_9.pth'
