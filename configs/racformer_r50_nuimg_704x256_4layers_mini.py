"""
RaCFormer Tier 2 Optimized Configuration for Jetson Orin AGX
- Reduced decoder layers: 6 -> 4 (shared weights, ~33% transformer speedup)
- Reduced temporal frames: 8 -> 4 (halves view transformer + radar processing)
- Queries kept at 900 (conservative, to isolate layer/frame impact)
- Uses nuScenes mini dataset paths

Usage:
    NUSCENES_VERSION=v1.0-mini python profile_inference.py \
        configs/racformer_r50_nuimg_704x256_tier2_mini.py \
        checkpoints/racformer_r50_f8.pth --bf16 --num-samples 10 --warmup 3 --detailed
"""
...
# TIER 2: Reduced decoder layers (6 -> 4)
# Shared-weight decoder, so reducing layers just means fewer forward passes.
# No weight mismatch — same decoder_layer weights reused fewer times.
num_layers = 4

# Keep original 8 frames (only layer reduction, fully weight-compatible)
num_frames = 8
...
# Original 6-layer: [0.08, 0.07, 0.06, 0.05, 0.04, 0.03]
# Evenly sampled 4-layer subset preserving start/end range:
d_region_list = [0.08, 0.06, 0.04, 0.03]

# Keep original query count (conservative)
num_clusters = 6
num_ray = 150
