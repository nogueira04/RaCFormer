...
class VoDMonoDataset(NuScenesMonoDataset):
    """Monocular 3D detection on VoD Dataset.
    ...
    def __init__(self,
        ...
        self.pts_prefix = 'lidar/training/velodyne'
        self.pts_radar_prefix = 'radar_5frames/training/velodyne'
    ...
    def _get_pts_filename(self, idx):
        ...
        """
        pts_filename = osp.join(self.data_root, self.pts_prefix,
                                f'{idx:05d}.bin')
        pts_radar_filename = osp.join(self.data_root, self.pts_radar_prefix,
                                f'{idx:05d}.bin')
        return pts_filename, pts_radar_filename


    def get_data_info(self, img_info):
        ...
        sample_idx = img_info['id']

        # TODO: consider use torch.Tensor only
        rect = np.asarray(img_info['rect']).astype(np.float32)
        Trv2c = np.asarray(img_info['Trv2c']).astype(np.float32)
        P2 = np.asarray(img_info['cam_intrinsic']).astype(np.float32)
        lidar2img = P2 @ rect @ Trv2c

        radar_rect = np.asarray(img_info['radar_rect']).astype(np.float32)
        radar_Trv2c = np.asarray(img_info['radar_Trv2c']).astype(np.float32)
        radar_P2 = np.asarray(img_info['radar_cam_intrinsic']).astype(np.float32)
        radar2img = radar_P2 @ radar_rect @ radar_Trv2c
        radar2cam = radar_rect @ radar_Trv2c

        pts_filename, pts_radar_filename = self._get_pts_filename(sample_idx)
        input_dict = dict(
            sample_idx=sample_idx,
            pts_filename=pts_filename,
            pts_radar_filename=pts_radar_filename,
            img_prefix=None,
    ...
    def prepare_train_img(self, idx):
        ...
        results = self.get_data_info(img_info)
        results.update(img_info=img_info, ann_info=ann_info)
    ...
    def prepare_test_img(self, idx):
        ...
        results.update(img_info=img_info)
        if self.proposals is not None:
            results['proposals'] = self.proposals[idx]
        self.pre_pipeline(results)
        return self.pipeline(results)
    ...
    def evaluate(self,
        ...
        """
        result_files, tmp_dir = self.format_results(results, pklfile_prefix, submission_prefix=submission_prefix)
